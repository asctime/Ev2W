                                  ...
                                  const gchar *uid)
{
  ...
  GtkWidget *focus = e_focus_tracker_get_focus (focus_tracker);
  GtkWindow *focus_window = e_focus_tracker_get_window (focus_tracker);
  g_debug ("Opening message \"%s\": \
    \nFocus Tracker is %s, addr: %p  Selectable: %s \
    \nSelectable status should be: %s \
    \nfocus widget addr: %p  focus window addr: %p \
    \nweb_view addr: %p  message window addr: %p \
    \nAction check. Focus copyclip addr: %p   Menu copyclip addr: %p\n", 
    title, E_IS_FOCUS_TRACKER (focus_tracker) ? "true" : "false",
    focus_tracker, E_IS_SELECTABLE (web_view) ? "true" : "false",
    E_IS_SELECTABLE (focus) ? "true" : "false",
    focus, focus_window, web_view, reader,
    e_focus_tracker_get_copy_clipboard_action (focus_tracker),
...
{
	EMailBrowser *browser;             /* gitlab #f9ffebc2  */
	EMFormatHTML *formatter;
	EMailReader *reader;
	EShellBackend *shell_backend;
	EShell *shell;
  ...
  browser = E_MAIL_BROWSER (object);
	...
	browser->priv->message_list = message_list_new (shell_backend);
	...
	action_group = browser->priv->action_group;
  ...
  browser->priv->main_toolbar = g_object_ref (widget);
	...
	browser->priv->search_bar = g_object_ref ((GtkWidget*)search_bar);

	g_signal_connect_swapped (
		search_bar, "changed",
		G_CALLBACK (em_format_queue_redraw), browser->priv->formatter);
...
e_mail_browser_close (EMailBrowser *browser)
{
  ...
  g_debug ("Browser widget closing: %p\n", browser);
