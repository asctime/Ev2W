...
static gchar* 
sanitize_filename (unsigned char *filename)
{
	gchar * sanitized_name;
	sanitized_name = g_path_get_basename ((const gchar*)filename);
...
void processTnef (TNEFStruct *tnef, const gchar *tmpdir) {
    ...
    if ((filename = MAPIFindUserProp (&(tnef->MapiProperties),
        ...
        if (strcmp((gchar *)filename->data, "IPM.Appointment") == 0) {
        ...
        if ((saveRTF == 1) && (tnef->subject.size > 0)) {
		    ...
		    g_free (file);
                    ...
                    if ((fptr = fopen(ifilename, "wb"))==NULL) {
                        g_debug("processTnef failed to write file %s\n", ifilename);
                    } else {
                        fwrite (buf.data,
                                sizeof (BYTE),
                                buf.size,
    ...
    while (p != NULL) {
		...
		/* process it. */
                ...
                if (filename->size == 1) {
                    ...
                    filename->data = (unsigned char*)tmpname;
                }
...
void saveVCard (TNEFStruct *tnef, const gchar *tmpdir) {
    ...
    if ((fptr = fopen(ifilename, "wb"))==NULL) {
      g_debug("saveVCard failed to write file %s\n", ifilename);
    } else {
...
void saveVCalendar (TNEFStruct *tnef, const gchar *tmpdir) {
    ...
    if ((fptr = fopen(ifilename, "wb"))==NULL) {
      g_debug("saveVCalendar failed to write file %s\n", ifilename);
    } else {
	...
	/* I think it only exists to connect future modification entries to
	   ...
	   every time, it should be ok :) */
        ...
        if ((filename=MAPIFindUserProp (&(tnef->MapiProperties),
            ...
            printRrule (fptr, (gchar *)filename->data, filename->size, tnef);
        }
...
void saveVTask (TNEFStruct *tnef, const gchar *tmpdir) {
    ...
    if ((fptr = fopen(ifilename, "wb"))==NULL) {
        g_debug("saveVTask failed to write file %s\n", ifilename);
    } else {
        fprintf(fptr, "BEGIN:VCALENDAR\n");
        fprintf(fptr, "VERSION:2.0\n");
...
void printRtf (FILE *fptr, variableLength *vl) {
    ...
    for (index = 0, byte=(gchar *)vl->data; index < vl->size; index++, byte++) {
        if (*byte == '}') {
            brace_ct--;
            key = 0;
            continue;
